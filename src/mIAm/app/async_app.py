...
from mIAm.postgres_db.async_postgres_db_manager import AsyncPostgresDBManager
...
# Page configuration and other constants remain the same
st.set_page_config(
...
@st.cache_resource
def get_db_manager() -> AsyncPostgresDBManager:
    """Get async database manager instance (cached)."""
    try:
        return AsyncPostgresDBManager(
...
async def login_user() -> None:
    ...
    try:
        ...
        user = await db_manager.authenticate_user(email=email, password=password)
        
        st.session_state.authenticated = True
        st.session_state.user_id = user["id"]
        st.session_state.user_profile = await db_manager.get_user_profile(user_id=user["id"])
...
async def register_user() -> None:
    ...
    try:
        ...
        db_manager = get_db_manager()
        user_id = await db_manager.register_user(
...
async def create_new_thread() -> None:
    ...
    try:
        ...
        thread_id = await db_manager.create_thread(
...
async def delete_thread(thread_id: str) -> None:
    ...
    try:
        db_manager = get_db_manager()
        await db_manager.delete_thread(thread_id)
...
# Modify main rendering functions to be async-friendly
...
def render_sidebar() -> None:
    ...
    async def _get_threads():
        db_manager = get_db_manager()
        return await db_manager.get_user_threads(user_id=st.session_state.user_id)

    with st.sidebar:
        st.markdown(f"### Welcome, {st.session_state.user_profile.get('first_name', 'User')}!")
        
        # Use st.button with async callback
        def _logout():
            asyncio.run(logout())
        ...
        with st.expander("💬 Create New Chat", expanded=True):
            with st.form("new_chat_form", clear_on_submit=True):
                ...
                if submit:
                    asyncio.run(create_new_thread())
        
        st.markdown("### Your Conversations")
        
        # Fetch threads using an async method
        threads = asyncio.run(_get_threads())
        ...
        if not threads:
            ...
            for thread in threads:
                with st.container():
                    ...
                    with col2:
                        def _select_thread(tid):
                            asyncio.run(select_thread(tid))
                        st.button("🔍", key=f"open_{thread['id']}", 
                                  on_click=_select_thread, 
                                  args=(thread['id'],), 
                                  help="Open this chat")
                    with col3:
                        def _delete_thread(tid):
                            asyncio.run(delete_thread(tid))
                        st.button("🗑️", key=f"delete_{thread['id']}", 
